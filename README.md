...
Amazingly, the update rules only have to be slightly tweaked to allow us to "unlearn" data, making it easy to create a trailing-window linear regression, keeping a log of the trailing input and output data, to incrementally unlearn data after a desired expiration period. The changed update rules for *unlearning* are:
